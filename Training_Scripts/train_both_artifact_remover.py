...
if __name__ == "__main__":
    ...
    x_train = np.concatenate((x_train, np.load("Datasets/brain_training_recon_limited.npy").astype(np.float32)),0)
    x_test = np.load("Datasets/chest_testing_recon_limited.npy").astype(np.float32)
    x_test = np.concatenate((x_test, np.load("Datasets/brain_testing_recon_limited.npy").astype(np.float32)),0)
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(net.state_dict(), "State_Dictionaries/both_artifact_remover")
        ...
        plt.legend()
        plt.savefig("Figures/both_artifact_removal_losses.png")
        plt.close()
