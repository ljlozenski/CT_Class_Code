...
if __name__ == "__main__":
    ...
    x_train = np.load("Datasets/chest_training.npy").astype(np.float32)
    x_train = np.concatenate((x_train, np.load("Datasets/brain_training.npy").astype(np.float32)),0)
    train_norm = np.mean(x_train**2)**(1/2)
    x_test =  np.load("Datasets/chest_testing.npy").astype(np.float32)
    x_test = np.concatenate((x_test, np.load("Datasets/brain_testing.npy").astype(np.float32)),0)
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(net.state_dict(), "State_Dictionaries/both_denoiser")
        ...
        plt.legend()
        plt.savefig("Figures/both_denoiser_losses.png")
        plt.close()
