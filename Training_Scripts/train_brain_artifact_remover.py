...
if __name__ == "__main__":
    ...
    x_train = np.load("Datasets/brain_training_recon_limited.npy").astype(np.float32)
    x_test = np.load("Datasets/brain_testing_recon_limited.npy").astype(np.float32)
    y_train = np.load("Datasets/brain_training.npy").astype(np.float32)
    y_test = np.load("Datasets/brain_testing.npy").astype(np.float32)
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(net.state_dict(), "State_Dictionaries/brain_artifact_remover")
        ...
        plt.legend()
        plt.savefig("Figures/brain_artifact_removal_losses.png")
        plt.close()
